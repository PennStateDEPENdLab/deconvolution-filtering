...
%Get dimensions of input
    ...
    if noiseOn
        ...
        for curr_node = 1:N        
            ...
            %Scale the noise to the proper SNR (amplitude-based)
%%             meanBLD = mean(BLDprn(curr_node,:).^2)
%%             mean((sqrt(meanBLD/SNphys)*zphys).^2)
            
%%             plot(BLDprn(curr_node,:));
%%             hold on;
%%             plot(sqrt(meanBLD/SNphys)*zphys,'r');
%% 
            ...
            %% 
%%             %%OLD
%%             %%%BLDprn(curr_node,:) = BLDprn(curr_node,:)+(mean(BLDprn(curr_node,:))/SNphys).*randn(1,numel(BLDprn(curr_node,:)));
%%             
%%             plot(BLDprn(curr_node,:),'g')
%%             hold off;
%%             drawnow;            
    ...
    desample_rate = FG/FO;
    [N,TS] = size(BLDprn);
    BLDobs = zeros(N,TS/desample_rate);
    for curr_node = 1:N
    ...
    if noiseOn
        ...
        %% for curr_node = 1:N
        %%     BLDobs(curr_node,:) = BLDobs(curr_node,:)+(mean(BLDobs(curr_node,:))/SNscan).*randn(1,numel(BLDobs(curr_node,:)));
        %% end
...
%%             mean((sqrt(meanBLD/SNscan)*zscan).^2)
%%             
%%             plot(BLDobs(curr_node,:));
%%             hold on;
%%             plot(sqrt(meanBLD/SNscan)*zscan,'r');
            
            %Compute combined signal
            BLDobs(curr_node,:) = BLDobs(curr_node,:)+ (sqrt(mean(BLDobs(curr_node,:).^2))/SNscan)*zscan;
            
%%             plot(BLDobs(curr_node,:),'g')
%%             hold off;
%%             drawnow;            
